...
class ResponseData implements \ArrayAccess {
	...
	public function __construct($type) {
		if ($type) {
			...
			throw new \Exception ( "Response data requires a type" );
	}
	...
	public function __isset($key) {
		return property_exists ( ( object ) $this->_data, $key );
	}
	...
	public function checkRequired() {
		foreach ( $this->_required as $key ) {
			if (! isset ( $this->{$key} ))
				throw new \Exception ( "Response data is missing key '{$key}'" );
		}
	}
